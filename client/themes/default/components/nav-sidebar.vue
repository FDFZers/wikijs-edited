<template lang="pug">
  div
    .pa-3.d-flex(v-if='navMode === `MIXED`', :class='$vuetify.theme.dark ? `grey darken-5` : `primary`')
      v-btn(
        depressed
        :color='$vuetify.theme.dark ? `grey darken-4` : `primary darken-1`'
        style='min-width:0;'
        @click='goHome'
    ...
    v-divider
    transition(name='m3-nav', mode='out-in')
      //-> Custom Navigation
      v-list.py-2(v-if='currentMode === `custom`', key='custom', dense, :class='color', :dark='dark')
      ...
      //-> Browse
      v-list.py-2(v-else-if='currentMode === `browse`', key='browse', dense, :class='color', :dark='dark')
...
<style lang="scss">
  // Smooth M3 transition when switching sidebar modes
  .m3-nav-enter-active,
  .m3-nav-leave-active {
    transition: opacity var(--m3-motion-med) var(--m3-easing-standard),
      transform var(--m3-motion-med) var(--m3-easing-standard);
  }
